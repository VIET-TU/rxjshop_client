...
export default function AvatarDropdown() {
  ...
  const [avatar, setAvatar] = useState<any>(avatarImgs[7]);
  ...
  useEffect(() => {
    const getDataUser = async () => {
      try {
        ...
        const data = await (
          await queryClient.fetchQuery<MeResponse>(["me"])
        )?.data?.data;
        if (data) {
  ...
  useEffect(() => {
    const refetchData = async () => {
      await queryClient.invalidateQueries(["me"]); // Gửi yêu cầu mới để lấy dữ liệu từ server
    };
  ...
  const logoutUserMutation = useMutation({
    ...
    onSuccess: (_, id) => {
      toast.success(`Logout sucssessfully`);
            ...
            >
              <Popover.Panel className="absolute z-10 w-screen max-w-[260px] px-4 mt-3.5 -right-10 sm:right-0 sm:px-0">
                <div className="overflow-hidden shadow-lg rounded-3xl ring-1 ring-black ring-opacity-5">
                  <div className="relative grid grid-cols-1 gap-6 px-6 bg-white py-7">
